        ...
        $.ajax({
            ...
            success: function(result) {
                ...
                if (resultObj.empty){
                    ...
                    newRow.html("<div class='product-detail'><div class='distance-bottom-more'><div class='row'><div class='col-md-5'><div class='product-images'><div><div class='image-imitation carouselContainer'><img class='center-block' src='seller/uploads/product/"+resultObj.images[0].image_name+"' alt='' /></div></div><div><div class='image-imitation carouselContainer'><img class='center-block' src='seller/uploads/product/"+resultObj.images[1].image_name+"' alt='' /></div></div><div><div class='image-imitation carouselContainer'><img class='center-block'  src='seller/uploads/product/"+resultObj.images[2].image_name+"' alt='' /></div></div></div></div><div class='col-md-7'><h2 class='font-bold m-b-xs'>"+resultObj.productName+"</h2><small>"+resultObj.shortDescription+"</small><div class='m-t-md'><h2 class='product-main-price'>IDR "+number_format(resultObj.productPrice)+"</h2></div><hr><h3>Product Description</h3><div class='small description text-muted'>"+resultObj.productDescription+"</div><hr><div><div class='btn-group'><button data-id='"+resultObj.productID+"' class='btn btn-primary btn-sm add-to'><i class='fa fa-cart-plus'></i> Add to cart</button><button class='btn btn-white btn-sm'><i class='fa fa-star'></i> Add to wishlist </button><button class='btn btn-white btn-sm'><i class='fa fa-envelope'></i> Contact with author </button></div></div></div></div></div><div class='ibox-footer'><span class='pull-right'>Last Update - <i class='fa fa-clock-o'></i> "+resultObj.update_at+"</span>Available Stock is "+resultObj.productQty+" pcs </div></div>");
